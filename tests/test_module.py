...
class TestModule(object):
    ...
    def test_warns_if_module_isnt_pinned_git(self, capsys):
    ...
    def test_passes_if_module_has_ref_git(self, capsys):
        with Wrap(self, ['tests/test_module/module_git_hasref.tf'], expect_exit=False):
            err = capsys.readouterr().err
            assert err == ''

    def test_warns_if_module_isnt_pinned_github(self, capsys):
        file = 'tests/test_module/module_github_missingref.tf'
        with Wrap(self, [file], expect_exit=True):
            err = capsys.readouterr().err
            assert ('Modules sourced from GitHub should be pinned in {}'.format(file)) in err

    def test_passes_if_module_has_ref_github(self, capsys):
        with Wrap(self, ['tests/test_module/module_github_hasref.tf'], expect_exit=False):
            err = capsys.readouterr().err
            assert err == ''
